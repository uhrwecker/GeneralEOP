...
def compute_4(s, chi, alpha, beta, r0, theta0, robs, thetaobs, phiobs):
    ...
    for phi0 in np.linspace(3 / 2 * np.pi, 2 * np.pi, num=50):
        ...
        # below:
        alpha, beta, flag = eop.find_critical_angles(half='right', amin=3.6, amax=4.2,
                                                     bmin=0.85, bmax=1.5)
...
def compute_2(s, chi, alpha, beta, r0, theta0, robs, thetaobs, phiobs):
    ...
    #    info[str(phi0)+'above'] = flag
    #    if flag:
    #        eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s1/2/')

    #for phi0 in np.linspace(3*np.pi / 4, np.pi, num=26):
    #    # below:
    #    sol = solver.ODESolver(s, theta0, phi0, chi, alpha, beta, r0, stop=40)
    #    eop = better_eop.EmitterObserverProblem(sol, robs, thetaobs, phiobs)
    #    alpha, beta, flag = eop.find_critical_angles(half='right', amin=2.9, amax=3.01,
    #                                                 bmin=1.2, bmax=1.4)
    #    info[str(phi0)+'below'] = flag
    #    if flag:
    #        eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s1/2/')

    phis = helping(r'D:/2021_04_13/s1/2/', np.pi/2, 3 * np.pi / 4, 25)[::-1]
    print(phis)
    for phi0 in [np.pi/2]:#phis:
        sol = solver.ODESolver(s, theta0, phi0, chi, alpha, beta, r0, stop=40)
        eop = better_eop.EmitterObserverProblem(sol, robs, thetaobs, phiobs)
        alpha, beta, flag = eop.find_critical_angles(half='right', amax=3.55, amin=3.7,
                                                     bmin=0.98, bmax=1.05)
        info[str(phi0) + 'below'] = flag
        if flag:
            eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s1/2/')
...
def compute_1(s, chi, alpha, beta, r0, theta0, robs, thetaobs, phiobs):
    ...
    #    info[str(phi0)+'above'] = flag
    #    if flag:
    #        eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s1/1/')
    ...
    phis = helping(r'D:/2021_04_13/s1/1/', np.pi / 4, np.pi / 2, 25)
    print(phis)
    for phi0 in phis:
        sol = solver.ODESolver(s, theta0, phi0, chi, alpha, beta, r0, stop=45)
        eop = better_eop.EmitterObserverProblem(sol, robs, thetaobs, phiobs)  #
        # above:
        alpha, beta, flag = eop.find_critical_angles(half='right', bmax=1.05, bmin=1.04,
                                                     amax=3.8, amin=3.7)
        info[str(phi0)+'below'] = flag
        if flag:
            eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s1/1/')##
