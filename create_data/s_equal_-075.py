...
def compute_4(s, chi, alpha, beta, r0, theta0, robs, thetaobs, phiobs):
    ...
    for phi0 in np.linspace(3 / 2 * np.pi, 2 * np.pi, num=50):
        ...
        #if flag:
        #    eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s-075/4/')

        # below:
        alpha, beta, flag = eop.find_critical_angles(half='right', amin=np.pi, amax=5,
                                                     bmin=0.9, bmax=1.5)
...
def compute_3(s, chi, alpha, beta, r0, theta0, robs, thetaobs, phiobs):
    ...
    for phi0 in np.linspace(np.pi, 3 * np.pi / 2, num=50):
        ...
        #info[str(phi0)+'above'] = flag
        #if flag:
        #    eop.save_to_csv(alpha, beta, r'D:/2021_04_13/s-075/3/')
...
def main():
    ...
    plotting(s, chi, r0, np.pi/2, a, b-0.00253)
