...
class EmitterObserverProblem:
    ...
    def find_critical_angles(self, half='right', amin=None, amax=None, bmin=None, bmax=None):
        ...
        alpha, beta, flag = self.bad_montecarlo(amin, amax, bmin, bmax, n=15, max_step=40)
    ...
    def save_to_csv(self, alpha, beta, folder='./'):
        ...
        if beta > np.pi / 2:
            tag = folder + '%.8f_%.8f_%.8f' % (data[:, 2][0], data[:, 4][0], data[:, 6][0])
        else:
            tag = folder + '%.8f' % data[:, 6][0]
    ...
    def _check_if_at_observer(self, r, t, p):
        ...
        if np.isclose(t[idx_r] % (np.pi), self.thetaobs, atol=1e-4, rtol=1e-4):
            tflag = True

        if np.isclose(p[idx_r] % (2 * np.pi), self.phiobs, atol=1e-4, rtol=1e-4):
            pflag = True
